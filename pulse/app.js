...
(function initParticleSystem() {
    ...
    function createParticle() {
        ...
        const tRect = titleEl ? titleEl.getBoundingClientRect() : { left: 0, top: 0, width: 120, height: 30 };
        ...
        imageFetchQueue.push(async () => {
            try {
                ...
                if (jsonResponse.ok && jsonResponse.result.file_path) {
                    ...
                    const delaySeconds = jsonResponse.parameters.retry_after * 1000 || 1500;
...
function applyThemePalette(themeMode) {
    ...
    if (themeMode === "Dark") {
        ...
        tg.setHeaderColor?.("#04060f");
        tg.setBackgroundColor?.("#04060f");
    } else {
...
function buildCategoryFilter() {
    ...
    categoriesExtract.forEach((catName) => {
        ...
        chipElement.onclick = () => {
            triggerLightHaptic();
            document.querySelectorAll(".cat-chip").forEach((cNode) => cNode.classList.remove("active"));
...
function renderUserInterface() {
    if (document.getElementById("v-home").classList.contains("active")) {
        ...
        const featuredSelection = queryResultStream.filter((premiumChecker) => premiumChecker.is_prem);
...
function createMediaCard(resourceItem, destinationContainer) {
    ...
    standardCardRoot.className = "item";
    const securityStatusTag = resourceItem.is_prem
        ? '<span style="color:var(--gold);font-weight:800;font-size:10px;">VIP</span>'
        : '<span style="color:var(--cyan);font-weight:800;font-size:10px;">Free</span>';
...
function openMediaDetails(databaseKey) {
    ...
    const pricingClassificationToken = targetMovieResource.is_prem
        ? '<span class="gold">💎 PREMIUM VIP</span>'
        : '<span class="free">⭐ FREE</span>';
    document.getElementById("mmet").innerHTML =
        `<span>📂 ${targetMovieResource.c || "Film/Video"}</span>&bull;<span>🎞 ${targetMovieResource.tp || 1} Ep/Part</span>&bull;${pricingClassificationToken}`;
    ...
    document.getElementById("btFIc").style.fill = checkBookmarkActive ? "var(--gold)" : "none";
    document.getElementById("btFIc").style.stroke = checkBookmarkActive ? "var(--gold)" : "currentColor";
    ...
    if (bsPostGlow) {
        bsPostGlow.style.background = checkBookmarkActive
            ...
            : "radial-gradient(ellipse, rgba(124,58,237,0.4), transparent 70%)";
    }
    ...
    tg.MainButton.text = targetMovieResource.is_prem
        ? "⭐ TONTON VIDEO (VIP) ⭐"
        : "▶ PUTAR MOVIE GRATIS";
